...
int main()
{
    ...
    while (1)
    {
        ...
        if (peek_n <= 0)
        {
            printf("Initial peek failed"); // something went wrong
            close(client_fd);
            continue;
        }
        ...
        {                                             // check if it's a file delete request
            memset(buffer, 0, BUFSIZE);               // clear buffer
            int n = read(client_fd, buffer, BUFSIZE); // read the full removef command
            if (n <= 0)
            {
                printf("Failed to read removef command"); // nothing read
                close(client_fd);
                continue;
            }
            buffer[n] = '\0';                                    // terminate string
            ...
            {
                printf("Delete failed");                        // file didn't exist or couldn't be removed
        ...
        // ✅ downltar
        if (strncmp(peekbuf, "downltar ", 9) == 0)
        {                                     // check for tar download
            memset(buffer, 0, BUFSIZE);       // clear buffer
        ...
        {                                             // if it’s a normal file download request
            memset(buffer, 0, BUFSIZE);               // clear buffer
            int n = read(client_fd, buffer, BUFSIZE); // read full path
            if (n <= 0)
            {
                printf("Failed to read downlf path");
                close(client_fd);
                continue;
            }
        ...
        if (n <= 0)
        {
            printf("Filename read failed"); // handle read error
            close(client_fd);
            continue;
        }
        ...
        if (n <= 0)
        {
            printf("Path read failed"); // handle read fail
            close(client_fd);
            continue;
        }
        buffer[n] = '\0'; // null terminate
        ...
        if (n <= 0)
        {
            printf("File size read failed"); // file size missing
            close(client_fd);
            continue;
        }
        buffer[n] = '\0';                           // end string
        ...
        if (!fp)
        {
            printf("fopen failed");                     // error opening
        ...
        {                                         // keep reading until full file is received
