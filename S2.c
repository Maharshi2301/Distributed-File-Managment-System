...
int main()
{
    ...
    while (1)
    {
        ...
        if (peek_n <= 0)
        {
            printf("Initial peek failed"); // if client disconnected or sent nothing
        ...
        {                                             // check if it's remove request
            memset(buffer, 0, BUFSIZE);               // clear buffer
            int n = read(client_fd, buffer, BUFSIZE); // read full command
            if (n <= 0)
            {
                printf("Failed to read removef command"); // read failed
                close(client_fd);
                continue;
            }
            buffer[n] = '\0';                                    // null terminate
            ...
            {
                printf("Delete failed");                        // show error in terminal
        ...
        {                                     // check if tar download requested
            memset(buffer, 0, BUFSIZE);       // clear buffer
        ...
        { // check if path sent means direct download
        ...
        if (n <= 0)
        {
            printf("Filename read failed"); // if nothing received
        ...
        if (!fp)
        {
            printf("fopen failed");                     // something went wrong
