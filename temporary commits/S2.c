...
int main() {
    ...
    while (1) {
        ...
        // If incoming request is a file delete or file download
        if (strncmp(peekbuf, "/home/", 6) == 0) {
            // Read full path
            memset(buffer, 0, BUFSIZE);
            int n = read(client_fd, buffer, BUFSIZE);
            if (n <= 0) {
                perror("Failed to read operation path");
                close(client_fd);
                continue;
            }
            buffer[n] = '\0';
            printf("File operation request: %s\n", buffer);
            ...
            if (remove(buffer) == 0) {
                ...
                write(client_fd, "Deleted", 7);
                close(client_fd);
                continue;
            }

            // If not deleted, fallback to downlf (open + send)
            fp = fopen(buffer, "rb");
            if (!fp) {
                write(client_fd, "0", 1);
                close(client_fd);
