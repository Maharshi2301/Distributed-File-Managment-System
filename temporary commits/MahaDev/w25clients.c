...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(input, "exit") == 0) break; // if user types exit, stop loop
        ...
        if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
            perror("Connection to server failed"); // server not reachable
            close(sock);
            continue;
        }

        // Handle uploadf command separately
        if (strncmp(input, "uploadf ", 8) == 0) { // check if it's upload command
            ...
            if (!fp) {
                perror("File open failed"); // file not found
                close(sock);
                continue;
            }
            ...
            // Send file content
            size_t n;
            while ((n = fread(buffer, 1, BUFSIZE, fp)) > 0) { // read in chunks
            ...
            if (bytes <= 0) {
                perror("Read failed or server disconnected"); // something went wrong
                close(sock);
                continue;
            }
	...
	// downlf logic
    if (strncmp(input, "downlf ", 7) == 0) { // check if user wants to download a file
        // Create a new socket
        ...
        if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
            perror("Connection to server failed"); // can't reach server
            close(sock);
            continue;
        }
        ...
        if (!filename) filename = filepath; // no '/' in path
    ...
    // removef logic
    else if (strncmp(input, "removef ", 8) == 0) { // check for delete request
        // Create new socket
        ...
        if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
            perror("Connection to server failed"); // server unreachable
            close(sock);
            continue;
        }
        ...
        if (bytes > 0) {
            ...
            perror("Failed to receive response"); // nothing came back
        }
    ...
    else if (strncmp(input, "dispfnames ", 11) == 0) { // check if it's a file list request
        // Create socket
    ...
    else if (strncmp(input, "downltar ", 9) == 0) { // handle tar file download
        char filetype[10];
        ...
        long received = 0;
        while (received < fsize) { // receive tar in chunks
        ...
        if (bytes > 0) {
            ...
            perror("Read failed or server disconnected"); // handle no reply
        }
