// S2.c – Handles uploadf, downlf, removef, and dispfnames for .pdf files
...
#define BUFSIZE 1024
    ...
    int main() {
        ...
        while (1) {
            ...
            if (strncmp(peekbuf, "dispfnames ", 11) == 0) {
                ...
                char listcmd[1100];
                snprintf(listcmd, sizeof(listcmd), "ls -p \"%s\" 2>/dev/null | grep '\\.pdf$' | sort", path);
            ...
            if (strncmp(peekbuf, "removef ", 8) == 0) {
                ...
                if (remove(filepath) == 0) {
                    ...
                    write(client_fd, "File deleted from S2\n", 22);
                } else {
                    perror("Delete failed");
                    write(client_fd, "File not found in S2\n", 22);
                }
                close(client_fd);
                continue;
            }
	...
	// ✅ downltar
        if (strncmp(peekbuf, "downltar ", 9) == 0) {
            ...
            char tarpath[512];
            snprintf(tarpath, sizeof(tarpath), "%s/%s", path, tarname);
