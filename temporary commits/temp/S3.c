// S3.c – Handles uploadf, downlf, removef, and dispfnames for .txt files
    ...
    int main() {
        ...
        while (1) {
            ...
            if (strncmp(peekbuf, "dispfnames ", 11) == 0) {
                ...
                char listcmd[1100];
                snprintf(listcmd, sizeof(listcmd), "ls -p \"%s\" 2>/dev/null | grep '\\.txt$' | sort", path);
