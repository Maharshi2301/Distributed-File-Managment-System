...
void prcclient(int client_fd) {
	...
	fflush(stdout);
        if (strncmp(buffer, "uploadf ", 8) == 0) {
            ...
            int resp_bytes = read(sock, filebuf, BUFSIZE);
            if (resp_bytes > 0) {
            ...
            if (strcmp(ext, ".c") == 0) {
                ...
                int target_port = 0;
                if (strcmp(ext, ".pdf") == 0) target_port = 21333;
                else if (strcmp(ext, ".txt") == 0) target_port = 21777;
                else if (strcmp(ext, ".zip") == 0) target_port = 22123;
                else {
                    write(client_fd, "0", 1);
                    continue;
                }
            ...
            else if (strcmp(ext, ".zip") == 0) target_port = 2213;
            else {
	    ...
	    // Step 4: get other types from S2, S3 and S4 and add it into the list
            char *pdf_files = get_remote_list(full_s2, 21333);
	    strcat(combined, pdf_files);
            char *txt_files = get_remote_list(full_s3, 21777);
	    strcat(combined, txt_files);
            char *zip_files = get_remote_list(full_s4, 22123);
