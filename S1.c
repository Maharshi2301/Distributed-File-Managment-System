...
void prcclient(int client_fd)
{
    ...
    while (1)
    {
        ...
        { // check if client asked to download a file
            char filepath[512];
            ...
            if (strncmp(filepath, "~S1", 3) == 0)
                ...
                snprintf(fullpath, sizeof(fullpath), "%s", filepath); // use path as it is
            ...
            else
            {
                ...
                int target_port = 0;
                if (strcmp(ext, ".pdf") == 0)
                    target_port = 21333; // s2
                else if (strcmp(ext, ".txt") == 0)
                    target_port = 21777; // s3
                else if (strcmp(ext, ".zip") == 0)
                    target_port = 22123; // s4
                else
                {
                ...
                {
                    printf("Failed to connect to backend"); // if connect fails
        ...
        { // check if client wants to delete file
            char filepath[512];
            ...
            if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0)
            {
                printf("Connection to backend server failed"); // show backend connect fail
            ...
            {
                write(client_fd, "You can't remove this file\n", 26); // if backend went silent
            }
        ...
        { // check if user asked to display file names
            char user_path[512];
