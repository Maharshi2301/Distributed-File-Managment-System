...
int main()
{
    ...
    while (1)
    {
        ...
        if (peek_n <= 0)
        {
            printf("Initial peek failed"); // nothing came or something broke
            close(client_fd);
            continue;
        }
        ...
        { // check if client wants file list
            memset(buffer, 0, BUFSIZE);
            int n = read(client_fd, buffer, BUFSIZE); // read command
            if (n <= 0)
            {
                printf("Failed to read dispfnames command"); // reading failed
                close(client_fd);
                continue;
            }
        ...
        { // intercept any zip delete attempt
            memset(buffer, 0, BUFSIZE);
            int n = read(client_fd, buffer, BUFSIZE); // read full removef cmd
            if (n <= 0)
            {
                printf("Failed to read removef command"); // nothing came
                close(client_fd);
                continue;
            }
            buffer[n] = '\0';
        ...
        {                                             // check if it’s a file download request
            ...
            if (n <= 0)
            {
                printf("Failed to read downlf path"); // something went wrong
                close(client_fd);
                continue;
            }
        ...
        if (!fp)
        {
            printf("fopen failed");                     // error if file can't be created
